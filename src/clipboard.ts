...
export async function getClipboardValue(page: Page): Promise<string> {
...
export async function setClipboardValue(page: Page, value: string): Promise<void> {
  verifyClipboardOrigin(page)

  await page.evaluate(evaluate_createClipboardElement, [CLIPBOARD_ELEMENT_ID])

  await page.locator(`#${CLIPBOARD_ELEMENT_ID}`).fill(value)
...
function evaluate_createClipboardElement(values: Array<string | number>): void {
...
function evaluate_removeClipboardElement(values: Array<string | number>): void {
  const [$id] = values as [string]

  const clipboardElement = document.getElementById($id)
  if (clipboardElement) document.body.removeChild(clipboardElement)
}

function verifyClipboardOrigin(page: Page): void {
