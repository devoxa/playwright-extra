...
export function $startsWith(value: string, dynamicSegments?: DynamicSegments): RegExp {
  return new RegExp('^' + applyDynamicSegments(escapeRegExp(value), dynamicSegments))
}

/** Create a RegExp that contains the value. */
export function $contains(value: string, dynamicSegments?: DynamicSegments): RegExp {
  return new RegExp(applyDynamicSegments(escapeRegExp(value), dynamicSegments))
}

/** Create a RegExp that ends with the value. */
export function $endsWith(value: string, dynamicSegments?: DynamicSegments): RegExp {
  return new RegExp(applyDynamicSegments(escapeRegExp(value), dynamicSegments) + '$')
}

function applyDynamicSegments(value: string, dynamicSegments = {} as DynamicSegments): string {
